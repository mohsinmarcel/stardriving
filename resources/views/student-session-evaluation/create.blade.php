...
<div class="row">
    <div class="col-12">
        <form action="{{route('student-session-evaluation.store')}}" method="POST" id="studentEvaluationForm">
            @csrf
            <div class="card">
                <div class="card-body">
                    <div class="header-title">Add Car Session Evaluation</div>
                    <hr>
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="row">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select id="student" class="form-control @error('student') is-invalid @enderror" name="student">
                                            ...
                                            @foreach ($students as $item)
                                                <option value="{{$item->id}}" {{old('student') == $item->id?'selected':''}}>{{$item->student_id}} / {{$item->first_name}} {{$item->last_name}}</option>
                                            @endforeach
                                        </select>
                                        @error('student')
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select id="teacher" class="form-control @error('teacher') is-invalid @enderror" name="teacher">
                                            ...
                                            @foreach ($teachers as $item)
                                                <option value="{{$item->id}}" {{old('teacher') == $item->id?'selected':''}}>{{$item->license_number}} / {{$item->first_name}} {{$item->last_name}}</option>
                                            @endforeach
                                        </select>
                                        @error('teacher')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                ...
                                <div class="col-md-4 col-lg-3">
                                    <div class="form-group">
                                        ...
                                        <select id="session" class="form-control @error('session') is-invalid @enderror" name="session" value="{{old('session')}}">
                                            <option value>--Select Session--</option>
                                            @php
                                                $sessions = \App\Constants\DatabaseEnumConstants::EVALUATION_SESSIONS;
                                            @endphp
                                            @foreach ($sessions as $item)
                                            <option value="{{$item}}" {{old('session') == $item?'selected':''}}>{{Str::title($item)}}</option>
                                            @endforeach
                                        </select>
                                        @error('session')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                    ...
                                    <div class="row">
                                            ...
                                            <div>
                                                @foreach ($strengths as $key => $item)
                                                    <div class="custom-control custom-checkbox checkbox-primary mb-2">
                                                        <input class="custom-control-input learner-strengths" data-value="{{$key}}" id="learner_strengths{{$key}}" name="learner_strengths[]" type="checkbox" value="{{$item->name}}" {{in_array($item->name, old('learner_strengths')??[])?'checked':''}}>
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <div>
                                                @foreach ($weaknesses as $key => $item)
                                                    <div class="custom-control custom-checkbox checkbox-primary mb-2">
                                                        <input class="custom-control-input learner-weaknesses" data-value="{{$key}}" id="learner_weaknesses{{$key}}" name="learner_weaknesses[]" type="checkbox" value="{{$item->name}}" {{in_array($item->name, old('learner_weaknesses')??[])?'checked':''}}>
                                        ...
                                        <div class="col-md-4">
                                            <div id="signatureDiv">
                                                <h5>
                                                    Draw your signature
                                                    <a class="text-right" href="#" id="clear">Clear</a>
                                                </h5>
                                    ...
                                    <div class="row">
                                        <div class="col-md-6">
                                            ...
                                            <div>
                                                @foreach ($strengths as $key => $item)
                                                    <div class="custom-control custom-checkbox checkbox-primary mb-2">
                                                        <input class="custom-control-input instructor-strengths" data-value="{{$key}}" id="instructor_strengths{{$key}}" name="instructor_strengths[]" type="checkbox" value="{{$item->name}}" {{in_array($item->name, old('instructor_strengths')??[])?'checked':''}}>
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <div>
                                                @foreach ($weaknesses as $key => $item)
                                                    <div class="custom-control custom-checkbox checkbox-primary mb-2">
                                                        <input class="custom-control-input instructor-weaknesses" data-value="{{$key}}" id="instructor_weaknesses{{$key}}" name="instructor_weaknesses[]" type="checkbox" value="{{$item->name}}" {{in_array($item->name, old('instructor_weaknesses')??[])?'checked':''}}>
